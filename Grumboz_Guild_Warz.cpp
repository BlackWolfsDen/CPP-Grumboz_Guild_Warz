									...
									if (sGGW->GWARZ[LocId].team == 2)
									{
										...
										if (player->GetItemCount(sGGW->GWCOMM[sGGW->SERVER_GUILD_ID].currency) >= Zoneprice) // Flag spawn is handled uniquely since i need to store its GUIDLow();
										{
											...
											if (spawnflagGUID)
											{
												...
												sGGW->UpdateGuildLocData("fs_time", sGGW->ConvertNumberToString(sWorld->GetGameTime()), LocId);
...
*/
					...
					if (ChatCache[1] == sGGW->GWCOMM[sGGW->SERVER_GUILD_ID].details_loc)
					{
						...
						ChatHandler(player->GetSession()).PSendSysMessage("%sGame Time|r:%s%u|r.", sGGW->GWCOMM[guild_id].color_1.c_str(), sGGW->GWCOMM[guild_id].color_2.c_str(), sWorld->GetGameTime());
						ChatHandler(player->GetSession()).PSendSysMessage("%s*************************************|r", sGGW->GWCOMM[guild_id].color_9.c_str());
						msg = GGW_MSG;
					}
				} // is minimum GM
...
public: GGW_GUILD_MAILBOX() : GameObjectScript("GGW_GUILD_MAILBOX"){ }
		...
		void UpdateAI(uint32 diff) // This will show first when a player clicks on a GameObject (Gossip)
		{
			...
			{
				UpdateGameObject(go, sGGW->GWCOMM[sGGW->sGGW->SERVER_GUILD_ID].mailbox_id, LocId);
			}
		}
	};
